...
globals [
  ...
  ticks-to-pulse ;; number of ticks before next pulse of toxin; this acts as a counter  pop-growth ;; keeping track of whether the population is growing or not - for use in deciding whether to dilute
...
to go
  ...
  ask turtles[
    ...
    if [toxin] of patch-here > 0 [ ;; check whether toxin is present
      ...
      if health < death-threshold [ ;; then, check whether health is below threshold
        die ;; if it is, die
      ]
    ]
    ...
    if (count neighbors with [not any? turtles-here]) > 0 [ ;; only reproduce if there's space
      if random-float 1 < growth-rate [ ;; higher growth rate -> higher probability of reproducing
        ...
        let new-generation [generation] of self + 1
        ;; mutate, if necessary
        if mutation-rate != 0 [ ;; only bother with this if mutation rate isn't 0
          if random-float 1 < mutation-rate [ ; do a random draw to see whether it's time to mutate
            ...
            if new-switch-rate < 0 [set new-switch-rate 0]
            if new-switch-rate > 1 [set new-switch-rate 1]
            set new-response-error [response-error] of self + random-float env-step - 0.5 * env-step
            if new-response-error < 0 [set new-response-error 0]
            if new-response-error > 1 [set new-response-error 1]
          ]
        ]
        ;; now reproduce
